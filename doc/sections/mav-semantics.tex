...
\item The key structure is duoidality.
...
law (the \RuleLabel{Sequence} rule in the proof system). We follow
...
\begin{proposition}
  If $(\bullet, i)$ is duoidal over $(\lhd, j)$ be pomonoids on
  $(A, \leq)$, and $(\bullet, i)$ distributes over $+$ (as in
  Proposition \ref{prop:closed-monoid-distrib}) and $+$ is duoidal
  over $(\lhd, j)$ (as in Proposition
  \ref{prop:closed-monoid-duoidal}), then
  $(\ClosedDay{\bullet}, \ClosedDay{i})$ is duoidal over
...
pullbacks simplifies to binary meets. For this section, we
...
\begin{proof}
  Combining the propositions above, in the special case of the units
  of the two pomonoids being equal.
\end{proof}
