...
\end{center}
Beautifully, internalising branching makes the symmetry between the axiom and cut plain to see. In BV, the axiom and cut rules are called \emph{interaction} and \emph{co-interaction}.
...
\begin{remark}
  ...
  Inferences rules are usually named with the combination of a letter and an up or down arrow, \eg \cref{rule:Interact} and \cref{rule:CoInteract} are $\iD$ and $\iU$, respectively.
\end{remark}

\begin{definition}\label{defn:normal}
  A derivation is \emph{normal} when it does not use \cref{rule:CoInteract} or any of the (\textsc{Co-X}) rules and its uses of \cref{rule:Interact} are restricted to \cref{rule:AtomInteract}, as defined by the following axiom:
  \begin{displaymath}
    \begin{array}{lcl@{\hspace{0.5cm}}|@{\hspace{0.5cm}}l}
